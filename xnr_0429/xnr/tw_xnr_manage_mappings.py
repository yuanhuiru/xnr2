...
def tw_xnr_mappings():
    ...
    exist_indice=es.indices.exists(index=tw_xnr_index_name)
    if not exist_indice:
      
        #create
        es.indices.create(index=tw_xnr_index_name,body=index_info,ignore=400)


def tw_xnr_fans_followers_mappings():
    index_info={
        'settings':{
            'number_of_replicas':0,
            'number_of_shards':5
    ...
    exist_indice=es.indices.exists(index=tw_xnr_fans_followers_index_name)
    if not exist_indice:
