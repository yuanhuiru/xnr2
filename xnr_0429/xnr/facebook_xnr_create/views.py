...
from utils import get_domain_info,get_role_info,get_role_sort_list,\
                ...
                get_xnr_info,get_show_domain,get_show_fb_xnr,get_nick_name_unique,\
                get_add_other_info,get_modify_userinfo,get_modify_base_info,get_xnr_info_new
import os
from xnr.global_config import PATH_ROOT

mod = Blueprint('facebook_xnr_create', __name__, url_prefix='/facebook_xnr_create')
...
# 返回虚拟人
@mod.route('/show_fb_xnr/')
def ajax_show_fb_xnr():
    submitter = request.args.get('submitter','admin@qq.com')
    fb_xnr_dict = get_show_fb_xnr(submitter)
    return json.dumps(fb_xnr_dict)
...
def ajax_save_step_three_1():
    ...
    task_detail['fb_mail_account'] = request.args.get('fb_mail_account','') # 邮箱
    task_detail['fb_phone_account'] = request.args.get('fb_phone_account','') # 手机号
