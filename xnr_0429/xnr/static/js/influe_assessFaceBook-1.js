...
$('.choosetime .demo-label input').on('click',function () {
    ...
    if (_val=='mize'){
        ...
        public_ajax.call_request('get',historyTotal_url,historyTotal);
        //曲线图 1
        var influe_7day_url='/facebook_xnr_manage/lookup_xnr_assess_info/?xnr_user_no='+ID_Num+
            '&start_time='+startTime+'&end_time='+end_time+'&assess_type=influence';
        public_ajax.call_request('get',influe_7day_url,influe_7day);
        //曲线图 2
        var influe_url='/facebook_xnr_assessment/'+midURL+'/?xnr_user_no='+ID_Num+lastURL;
...
var scoreUrl='/facebook_xnr_assessment/influence_mark/?xnr_user_no='+ID_Num;
...
function influe(data) {
    ...
    if (isEmptyObject(data)||isEmptyObject(data['day_num'])){
        ...
        for (var i in data['day_num']){
            ...
            else if (i=='fans'){legend.push('好友');fansData=publicData(data['day_num'][i])}
    ...
    if (isEmptyObject(data)||isEmptyObject(data['total_trend'])){
        ...
        for (var m in data['total_trend']){
            ...
            else if (m=='fans'){legend2.push('好友总数');totalFans2=publicData(data['total_trend'][m])}
    ...
    if (isEmptyObject(data)||isEmptyObject(data['growth_rate'])){
        ...
        for (var h in data['growth_rate']){
            ...
            else if (h=='fans'){growLEG.push('好友增长率');growthfansData=publicData(data['growth_rate'][h])}
