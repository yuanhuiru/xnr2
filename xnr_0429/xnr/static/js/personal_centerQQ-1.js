        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {
                    if (row.xnr_user_no == '' || row.xnr_user_no == 'null' || row.xnr_user_no == 'unknown'||!row.xnr_user_no) {
                        ...
                        return row.xnr_user_no+'&nbsp;&nbsp;<a style="cursor: pointer;color:white;" onclick="lookStatus(\''+row.xnr_user_no+'\')" title="查看登陆状态"><i class="icon icon-eye-open"></i></a>';
                    };
                }
            },
            {
           ...
           /* {
                ...
                valign: "middle",//垂直
				formatter: function (value, row, index) {
					if (row.login_status=='True'){return '在线'}else if(row.login_status=='False'){return '离线'}else{return '未知'}
                },
            },*/
            {
                title: '操作',//标题
                ...
                formatter: function (value, row, index) {
                    ...
                    var ld;
					if (row.login_status=='True'){ld= '在线'}else if(row.login_status=='False'){ld='离线'}else{ld= '未知'}
...
//查看登陆状态
function lookStatus(xnrid){
	var status_url='/qq_xnr_manage/login_status/?xnr_user_no='+xnrid;
	$.ajax({
            type:'GET',
            url:status_url,
            async:true,
            ...
            success:function (data) {
                ...
                if (data=='True'){
					...
					s='您的QQ号码已经离线。'
                }else {s='抱歉，您的QQ号码登陆状态未知，请稍后再试。'}
        		 $('#succee_fail #words').text(s);
...
$('.optSureadd').on('click',function () {
    ...
    if (qnum==''||qgp==''||qname==''||qpower==''||qgpName==''||qgpBEIZHU==''||qremark==''){
