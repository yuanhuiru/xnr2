...
$('.choosetime .demo-label input').on('click',function () {
    ...
    if (_val=='mize'&&sh!=0){
        ...
        if (mid=='show_history_count'){
            if (_val==0){
                his_timing_task_url='/facebook_xnr_manage/show_history_count/?xnr_user_no='+ID_Num+'&type=today&start_time=0&end_time='+end_time;
            }else {
            ...
            if (_val==0){
                his_timing_task_url='/facebook_xnr_manage/'+mid_2[0]+'/?xnr_user_no='+ID_Num+'&'+mid_2[2]+'='+conTP.join(',')+
            ...
            his_timing_task_url='/facebook_xnr_manage/'+mid+'/?xnr_user_no='+ID_Num+'&start_time='+startTime+'&end_time='+end_time;
        }
...
//定时发送任务列表
var timingTask_url='/facebook_xnr_manage/show_timing_tasks/?xnr_user_no='+ID_Num+'&start_time='+todayTimetamp()+'&end_time='+end_time;
...
function lookRevise(_id,ST) {
    __ST=ST;
    var saw_url='/facebook_xnr_manage/wxnr_timing_tasks_lookup/?task_id='+_id;
...
function sureModify() {
    ...
    var againSave_url='/facebook_xnr_manage/wxnr_timing_tasks_change/?task_id='+id+'&task_source='+task_source+
...
function revoked(_id) {
    escTxt=1;
    var delTask_url='/facebook_xnr_manage/wxnr_timing_tasks_revoked/?task_id='+_id;
...
$('#container .rightWindow .news #myTabs li').on('click',function () {
    ...
    if (tm){
        if (tm!='mize'){
            liNews_url='/facebook_xnr_manage/'+middle[0]+'/?xnr_user_no='+ID_Num+'&'+middle[2]+'='+$params.join(',')+
...
$('#container .rightWindow .oli .news #content input').on('click',function () {
    ...
    if (tm){
        if (tm!='mize'){
            againHistoryNews_url='/facebook_xnr_manage/'+typeDown+'/?xnr_user_no='+ID_Num+'&'+MID+'='+content_type+
                '&start_time='+getDaysBefore(tm)+'&end_time='+end_time;
        }else {
            var s=$(this).parents('.news').prev().find('#start_3').val();
            var d=$(this).parents('.news').prev().find('#end_3').val();
        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var str=
						...
						'           <div class="center_3">'+
                        '               <span class="cen3-1" onclick="retweet(this,\'日常发帖\')" style="display: '+showHide2+';"><i class="icon icon-share"></i>&nbsp;&nbsp;转发 <b style="'+C3+'"></b></span>'+
                        '               <span class="cen3-2" onclick="showInput(this)" style="display:'+showHide2+';"><i class="icon icon-comments-alt"></i>&nbsp;&nbsp;评论<b style="'+C3+'"></b></span>'+
...
function dialogue(_this) {
    ...
    var dialogue_url='/facebook_xnr_manage/show_comment_dialog/?mid='+mid;
...
function joinHeavy(){
	var useradd_url='/facebook_xnr_operate/trace_follow/?xnr_user_no='+ID_Num+'&uid_string='+$id_;
...
$('.fansSEN .demo-label input').on('click',function () {
    var orderType=$(this).val();
    var clikcFans_url='/facebook_xnr_manage/wxnr_list_fans/?user_id='+ID_Num+'&order_type='+orderType;
...
//     var details_url='/facebook_xnr_manage/lookup_detail_weibouser/?uid='+_id;
...
function focus_ornot(_id,mid) {
    var focusNOT_url='/facebook_xnr_manage/'+mid+'/?xnr_user_no='+ID_Num+'&uid='+_id;
