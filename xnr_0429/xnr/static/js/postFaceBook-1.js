...
$('.choosetime .demo-label input[name="time1"]').on('click',function () {
    ...
    var flow_faw_url;
    if (_val=='no'){
        flow_faw_url='/facebook_xnr_operate/show_retweet_timing_list_future/?xnr_user_no='+ID_Num;
        public_ajax.call_request('get',flow_faw_url,flow_faw);
    }else {
        ...
        if (_val=='mize'){
            ...
            $('#end_1').hide();
            $('.sureTime').hide();
            flow_faw_url='/facebook_xnr_operate/show_retweet_timing_list/?xnr_user_no='+ID_Num+'&start_ts='+startTime+
                '&end_ts='+end_time;
            public_ajax.call_request('get',flow_faw_url,flow_faw);
        }
    }
...
$('.sureTime').on('click',function () {
    ...
    if (s==''||d==''){
        ...
        var his_timing_task_url='/facebook_xnr_operate/show_retweet_timing_list/?xnr_user_no='+ID_Num+'&start_ts='+(Date.parse(new Date(s))/1000)+
...
function addHeavySure() {
    ...
    if (!uid_name){
        ...
        if(m=='uid_string'){
            ...
            if(reg.test(m)){
                useradd_url='/facebook_xnr_operate/trace_follow/?xnr_user_no='+ID_Num+'&'+m+'='+uid_name;
            }else {
...
$('#theme-2 .demo-label input').on('click',function () {
    ...
    var the_url='/facebook_xnr_operate/hot_recommend_tweets/?topic_field='+the+'&sort_item='+theSort;
...
$('#theme-3 .demo-label input').on('click',function () {
    ...
    var the_url='/facebook_xnr_operate/hot_recommend_tweets/?topic_field='+theSort+'&sort_item='+the;
    public_ajax.call_request('get',the_url,hotWeibo)
});
var hotWeiboUrl='/facebook_xnr_operate/hot_recommend_tweets/?topic_field=民生类_法律&sort_item=timestamp';
// public_ajax.call_request('get',hotWeiboUrl,hotWeibo);
        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var str=
						...
						'           <div class="_translate" style="display: none;"><b style="color: #f98077;">译文：</b><span class="tsWord"></span></div>'+
                        '           <div class="center_3">'+
                        '               <span onclick="related(this)" title="事件子观点及相关帖子"><i class="icon icon-stethoscope"></i>&nbsp;&nbsp;事件子观点及相关发帖</span>'+
                        '               <span onclick="copyPost(this)" title="复制"><i class="icon icon-copy"></i>&nbsp;&nbsp;复制</span>'+
                        '               <span onclick="retweet(this,\'hot_post\')" title="分享数"><i class="icon icon-share"></i>&nbsp;&nbsp;分享&nbsp;</span>'+
...
$('#theme-4 .demo-label input').on('click',function () {
    var the=$(this).val();
    var the_url='/facebook_xnr_operate/bussiness_recomment_tweets/?xnr_user_no='+xnrUser+'&sort_item='+the;
    public_ajax.call_request('get',the_url,businessWeibo)
});
var busWeiboUrl='/facebook_xnr_operate/bussiness_recomment_tweets/?xnr_user_no='+xnrUser+'&sort_item=timestamp';
// public_ajax.call_request('get',busWeiboUrl,businessWeibo);
        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var str=
                        ...
                        '               <span class="cen3-1" onclick="retweet(this,\'business_post\')"><i class="icon icon-share"></i>&nbsp;&nbsp;分享</span>'+
