...
from xnr.global_utils import r, tw_xnr_index_name, tw_xnr_index_type,\
                            ...
                            tw_xnr_max_no
from xnr.parameter import tw_domain_ch2en_dict,tw_domain_en2ch_dict, TRANS_PATH, TW_OP_PATH
...
from xnr.time_utils import get_twitter_flow_text_index_list as get_flow_text_index_list, datetime2ts
...
def get_show_tw_xnr(submitter):
    ...
    if es_results:
        for result in es_results:
            result = result['_source']
            tw_xnr_dict[result['xnr_user_no']] = result['nick_name']
    return tw_xnr_dict
...
def get_role2feature_info(domain_name,role_name):
    domain_pinyin = pinyin.get(domain_name,format='strip',delimiter='_')
    role_name_en = tw_domain_ch2en_dict[role_name]
    _id = domain_pinyin + '_' + role_name_en
    try:
        es_result = es.get(index=tw_role_index_name,doc_type=tw_role_index_type,id=_id)['_source']
...
def get_recommend_step_two(task_detail):
    ...
    for result in member_uids_results:
        if result['found'] == True:
            result = result['_source'] 
            person_url = "https://twitter.com/" + result['userscreenname']
...
def get_modify_userinfo(task_detail):
    ...
    try:
        tw_mail_account = xnr_result['tw_mail_account']
    except:
        tw_mail_account = ''
    try:
        tw_phone_account = xnr_result['tw_phone_account']
    except:
        tw_phone_account = ''
...
def get_xnr_info_new(xnr_user_no):
    results = es.get(index=tw_xnr_index_name,doc_type=tw_xnr_index_type,id=xnr_user_no)['_source']
    return results
...
def get_role_info(domain_pinyin,role_name):
    ...
    role_info = es.get(index=tw_domain_index_name,doc_type=tw_domain_index_type,id=role_info_id)['_source']
    return role_info
