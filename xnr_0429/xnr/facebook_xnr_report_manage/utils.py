...
'''
facebook report management
'''
import sys
import json
import xlwt
import time
from xnr.global_utils import es_xnr,facebook_report_management_index_name_pre,facebook_report_management_index_type

from xnr.parameter import MAX_SEARCH_SIZE,DAY

from xnr.time_utils import ts2datetime

from xnr.reportManage.fbreport import Report
...
def show_report_content(report_type,start_time,end_time):
    ...
    report_management_index_name = get_xnr_reportment_index_listname(facebook_report_management_index_name_pre,start_time,end_time)
    result=[]
    try:
        results=es_xnr.search(index=report_management_index_name,doc_type=facebook_report_management_index_type,body=query_body)['hits']['hits']
        for item in results:
...
def output_excel_word(id_list,out_type,report_timelist):
    ...
    for item in report_timelist:
        index_name = facebook_report_management_index_name_pre + ts2datetime(int(item))
