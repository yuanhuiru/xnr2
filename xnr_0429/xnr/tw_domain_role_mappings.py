...
def domain_base_mappings():
    ...
    if not es.indices.exists(index=tw_domain_index_name):
        print es.indices.create(index=tw_domain_index_name,body=index_info,ignore=400)
...
def role_base_mappings():
    ...
    if not es.indices.exists(index=tw_role_index_name):
        print es.indices.create(index=tw_role_index_name,body=index_info,ignore=400)
...
def example_model_mappings():
    ...
    if not es.indices.exists(index=tw_example_model_index_name):
        print es.indices.create(index=tw_example_model_index_name,body=index_info,ignore=400)
