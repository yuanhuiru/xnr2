...
from xnr.parameter import HOT_WEIBO_NUM,MAX_VALUE,MAX_SEARCH_SIZE,DAY,FLOW_TEXT_START_DATE,REMIND_DAY,MAX_WARMING_SIZE,EVENT_COUNT,NOFOLLOWER_INFLUENCE_MIN_JUDGE
...
def lookup_history_user_warming(xnr_user_no,start_time,end_time):
    ...
    try:
        ...
        for item in temp_results:
            item['_source']['content']=json.loads(item['_source']['content'])
...
#the version was abandoned in 2018-06-16
'''
...
def lookup_history_event_warming(xnr_user_no,start_time,end_time):
    ...
    try:
        ...
        results=[]
        for item in temp_results:
            item['_source']['main_user_info'] = json.loads(item['_source']['main_user_info'])
            item['_source']['main_weibo_info'] = json.loads(item['_source']['main_weibo_info'])
            item['_source']['_id']=item['_id']
            results.append(item['_source'])
...
def show_event_warming(xnr_user_no,start_time,end_time):
    ...
    warming_list = lookup_history_event_warming(xnr_user_no,start_time,end_time)
