...
import json
sys.path.append('../')
from global_utils import es_xnr_2 as es,facebook_flow_text_index_name_pre,facebook_flow_text_index_type,\
...
def facebook_user_mappings(index_name=facebook_user_index_name):
    index_info = {
        'settings':{
            'number_of_replicas':0,
            'number_of_shards':5
        ...
        'mappings':{
            facebook_user_index_type:{
                'properties':{
                    ...
                    'chinese_user':{ # 是否为中文用户，方便翻译前的判断   0 - 否  1 - 是
                        'type':'short'
                    },
                    'work':{
                        'type':'string',
