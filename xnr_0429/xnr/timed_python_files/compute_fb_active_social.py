...
from global_utils import weibo_hot_keyword_task_index_name,weibo_hot_keyword_task_index_type,\
                            ...
                            weibo_xnr_index_name,weibo_xnr_index_type,flow_text_index_name_pre,\
                            ...
                            fb_bci_index_name_pre ,fb_bci_index_type,fb_portrait_index_name as portrait_index_name,\
                            fb_portrait_index_type as portrait_index_type,\
                            es_user_profile,es_retweet,be_retweet_index_name_pre,be_retweet_index_type
...
def load_user_index_res():
    ...
    avg_sort_uid_dict = {}
    current_date = ts2datetime(time.time()-2*24*3600)
#     date = ''.join(current_date.split('-'))
    date = current_date
    index_name = fb_bci_index_name_pre + date
    query_body = {
        'query': {
            'match_all': {},
        },
...
#         'sort': {"influence": {"order": "desc"}},
    ...
    uid_list = []
    try:
        search_res = es_user_portrait.search(index=index_name, doc_type=fb_bci_index_type, body=query_body)['hits']['hits']
        for r in search_res:
            uid = r['_id']
            uid_list.append(uid)
            avg_sort_uid_dict[uid] = {}
#             avg_sort_uid_dict[uid]['sort_item_value'] = round(r['_source']['user_index'],2)
#             avg_sort_uid_dict[uid]['sort_item_value'] = round(r['_source']['influence'],2)
            avg_sort_uid_dict[uid]['sort_item_value'] = round(r['_source']['active'],2)
    except Exception,e:
        print e
    ...
    for uid in uid_list:
        ...
        es_results = es_user_portrait.search(index=portrait_index_name,doc_type=portrait_index_type,body=query_body)['hits']['hits']
        print 'es_results', len(es_results)
        if es_results:
            #print 'portrait--',es_results[0]['_source'].keys()
            for item in es_results:
                uid = item['_source']['uid']
