...
from global_utils import es_xnr_2 as es,twitter_flow_text_index_name_pre,twitter_flow_text_index_type,\
...
def influence_active(uid,index_name):
	...
	#index_name = twitter_flow_text_index_name_pre + ts2datetime(current_time)

	es_count = es.count(index=index_name,doc_type=twitter_flow_text_index_type,body=query_body)
...
def influence_propagate(tid,index_name):
	query_body = {
		'query':{
			'term':{'tid':tid}
		},
	...
	#index_name = twitter_flow_text_index_name_pre + ts2datetime(current_time)
	search_results = es.search(index=index_name,doc_type=twitter_count_index_type,\
...
if __name__ == '__main__':
    '''
	current_date_end = '2018-06-14'
	current_date_start = '2018-05-01'
