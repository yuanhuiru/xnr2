...
from global_utils import es_xnr as es_text
...
def filter_mid(mid_list):
    ...
    for i in range(l_1000+1):
        ...
        if tmp:
            es_results = es_xnr.mget(index="tw_social_sensing_text", doc_type="text", body={"ids":tmp}, _source=False)["docs"]
...
def social_sensing(task_detail):
    ...
    if all_origin_list:
        ...
        for mid in all_origin_list:
            retweet_count = es_text.count(index=index_list, doc_type="text", body={"query":{"bool":{"must":[{"term":{"fid": mid}}]}}})["count"]
            comment_count = es_text.count(index=index_list, doc_type="text", body={"query":{"bool":{"must":[{"term":{"fid": mid}}]}}})["count"]
            tmp = dict()
