...
def rpop_compute_intelligent_writing():
	...
	#except:
	#    r_r.lpush(writing_task_queue_name,json.dumps(task_detail))	
...
def find_flow_texts(task_source,task_id,event_keywords):
    ...
    if task_source == 'weibo':
        ...
        # test
	#current_time = datetime2ts('2018-05-13')   
    ...
    if task_source == 'weibo':
        ...
        if tweets_results:
            for item in tweets_results:
                item = item['_source']
                weibo_list = []
		weibo_list.append(item['mid'])
		weibo_list.append(item['text'])
		weibo_list.append(item['uid'])
		weibo_list.append(item['timestamp'])
		#nick_name,photo_url = uid2nick_name_photo(item['uid']
		weibo_list.append(item['uid'])
		weibo_list.append(item['retweeted'])
		weibo_list.append(item['comment'])
                tweets_list.append(weibo_list)
                #weibo = item['text']
...
def get_models_text(task_id,task_source,opinion_keywords_list):
    ...
    try:
        ...
        model_text_dict = {}	
        model_text_dict['model_text_pos'] = ''
        model_text_dict['model_text_neg'] = ''
        model_text_dict['model_text_news'] = ''
...
def opinion_relevance_main(task_source,task_id,xnr_user_no,opinion_keywords_list):
    ...
    for result in results:
        ...
        item_result = opinion_relevance(opinion_keywords_list,corpus_name)
