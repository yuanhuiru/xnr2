...
import copy
import os
from social_v_sensing import social_sensing_task 
reload(sys)
sys.path.append("../../")
...
from global_utils import index_V_sensing as index_sensing,type_V_sensing as type_sensing
...
def create_task_list():
    ...
    file_path = os.path.join(current_path, 'task_v_list.py')
    if S_TYPE == 'test':
    ...
    if search_results:
        for iter_item in search_results:
            ...
            task.append(now_ts)
            r.lpush("task_v_name", json.dumps(task))
            count += 1
